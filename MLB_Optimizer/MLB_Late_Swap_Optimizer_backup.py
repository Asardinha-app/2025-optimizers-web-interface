...
from MLB_Optimizer import Config as MLBConfig, Player, Lineup

# Import late swap components
from late_swap.core import SwapAnalysis, analyze_lineup_for_swaps, should_skip_lineup
from late_swap.utils import load_template_lineups, export_swapped_lineups, setup_logger
...
class LateSwapOptimizer:
    ...
    def _parse_lineup_from_csv(self, lineup_data: Dict) -> Optional[Lineup]:
        ...
        try:
            ...
            try:
                import simple_lineup_parser
            except Exception as e:
                self.logger.error(f"Failed to import simple_lineup_parser: {e}")
                return None
            ...
            try:
                lineup_result = simple_lineup_parser.parse_lineup_simple(
                    clean_lineup_data,
                    self.players, 
                    self.config.FD_POSITION_ORDER
                )
            except Exception as e:
                self.logger.error(f"Failed to call simple_lineup_parser.parse_lineup_simple: {e}")
                import traceback
