...
class Config:
    ...
    MAX_PRIMARY_STACK_PCT = 0.1675 # Default max exposure for primary stacks
    MAX_SECONDARY_STACK_PCT = 0.1006 # Default max exposure for secondary stacks
    MAX_SALARY = 35000
    MAX_ATTEMPTS = 1000
    ...
    {"pitchers": ["Jesus Luzardo"], "stacks": ["TB","NYY"]},
...
class MLBOptimizer:
    """Main MLB Optimizer class for generating lineups."""
    
    def __init__(self, config=None):
        """Initialize the MLB Optimizer with optional custom configuration."""
    ...
    def load_data(self, data_file=None):
        ...
        file_path = data_file or self.config.DATA_FILE
        df, self.ro8_9_ids = load_and_clean_data(file_path)
        self.players = create_player_objects(df)
        self.teams = list(set(p.team for p in self.players))
        
        # Validate data
        if len(self.players) < 9:
            raise ValueError(f"Insufficient players ({len(self.players)}) for lineup optimization. Need at least 9 players.")
    ...
    def generate_lineups(self, num_lineups=None, max_attempts=None):
        """Generate optimized lineups."""
        if not self.players:
            raise ValueError("No player data loaded. Call load_data() first.")
        
        num_lineups = num_lineups or self.config.NUM_LINEUPS_TO_GENERATE
        ...
        primary_stack_counts = {}
        secondary_stack_counts = {}
        recent_primary_teams = {team: 0 for team in self.teams}
        recent_secondary_teams = {team: 0 for team in self.teams}
        attempt = 0
        
        # Generate lineups
        while (len(generated_lineups) < num_lineups and 
               attempt < max_attempts):
            attempt += 1
            model, variables = create_lineup_model(
                self.players, self.teams, primary_stack_counts, secondary_stack_counts,
                recent_primary_teams, recent_secondary_teams, used_lineups_sets,
                self.ro8_9_ids, len(generated_lineups)
            )
            lineup = solve_lineup(model, self.players, self.teams, variables)
            if not lineup:
                continue
    ...
    def export_lineups(self, lineups, output_path=None):
        """Export lineups to CSV file."""
        if not lineups:
            print("No lineups to export.")
            return
        
        output_path = output_path or "/Users/adamsardinha/Desktop/FD_MLB_Lineups.csv"
        export_to_csv(lineups, output_path)
