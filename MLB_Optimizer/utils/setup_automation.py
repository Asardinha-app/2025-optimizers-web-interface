#!/usr/bin/env python3
"""
Setup Automation for MLB Optimizer Weekly Updates
...
class AutomationSetup:
    ...
    def __init__(self):
        ...
        self.optimizer_dir = self.script_dir
        self.cron_file = Path("mlb_optimizer_cron")
...
<dict>
    ...
    def setup_linux_automation(self) -> bool:
        ...
        try:
            ...
            cron_entry = f"0 9 * * 1 cd {self.script_dir} && {sys.executable} auto_update.py >> {self.log_dir}/weekly_update.log 2>&1"
            ...
            if "mlboptimizer" not in current_crontab:
                # Add our cron job
                new_crontab = current_crontab + f"\n# MLB_Optimizer Weekly Update\n{cron_entry}\n"
                
                # Write new crontab
                with open(self.cron_file, 'w') as f:
                    f.write(new_crontab)
                
                subprocess.run(['crontab', str(self.cron_file)], check=True)
...
Register-ScheduledTask -TaskName "MLB_Optimizer Weekly Update" -Action $action -Trigger $trigger -Settings $settings -Principal $principal -Description "Weekly update for MLB_Optimizer"
"""
...
"""
    ...
    def create_notification_script(self) -> bool:
        """Create a notification script for update status."""
        notification_script = f"""#!/usr/bin/env python3
\"\"\"
MLB_Optimizer Update Notifier
...
def check_update_status():
    ...
    log_file = Path("MLB_Optimizer/logs/weekly_update.log")
        ...
        try:
            with open(log_file, 'r') as f:
                ...
                if lines:
                    ...
                    if "successfully" in last_line.lower():
                        send_notification("MLB_Optimizer", "Weekly update completed successfully")
                        return True
                    else:
                        send_notification("MLB_Optimizer", "Weekly update may have issues - check logs")
                        return False
        except Exception as e:
            send_notification("MLB_Optimizer", f"Error checking update status: {{e}}")
            return False
    
    send_notification("MLB_Optimizer", "No update logs found")
    return False
...
"""
        ...
        try:
            notify_file = self.script_dir / "notify_update.py"
            with open(notify_file, 'w') as f:
                f.write(notification_script)
            
            os.chmod(notify_file, 0o755)
            
            print(f"✅ Notification script created: {notify_file}")
            return True
            
        except Exception as e:
            print(f"❌ Failed to create notification script: {e}")
            return False
    ...
    def setup_automation(self) -> bool:
        ...
        if success:
            # Create monitoring and notification scripts
            self.create_monitoring_script()
            self.create_notification_script()
            
            # Create configuration file
            config = {
                'platform': platform,
                'setup_date': datetime.now().isoformat(),
                'update_schedule': 'Weekly (Monday 9:00 AM)',
                'log_directory': str(self.log_dir),
                'scripts': {
                    'auto_update': str(self.script_dir / 'auto_update.py'),
                    'monitor': str(self.script_dir / 'monitor_automation.py'),
                    'notify': str(self.script_dir / 'notify_update.py')
                }
            }
            
            config_file = self.script_dir / 'automation_config.json'
            ...
            print(f"🔍 Monitor automation with: python monitor_automation.py")
            
            # Test the setup
            print(f"\n🧪 Testing automation setup...")
            test_result = subprocess.run([
                sys.executable, 'auto_update.py', '--check-only'
            ], capture_output=True, text=True, cwd=self.script_dir)
            
            if test_result.returncode == 0:
                print("✅ Automation test passed!")
            else:
                print("⚠️  Automation test had issues - check configuration")
...
def main():
    ...
    if setup.setup_automation():
        ...
        print("\n📋 Next Steps:")
        print("1. Monitor the first weekly update (next Monday at 9:00 AM)")
